...
def vib_live(pbint, ligne_shoulders, ligne_shoulders_init, dist_shoulder, dist_shoulder_init,  dist_hip, dist_hip_init, ligne_hips_init, ligne_hips, angle_knee, ind_other_side, angle_hip, dist_knee, dist_knee_init ) :
    ...
    #crit is the error criteria and inter is the range of errors [min,max]
        ...
        #normalization of the error range between 150 and 255 for the back and 100 and 400 for the knees
        vib = min_norm[vib_id] + (inter[1]-crit) * (max_norm[vib_id]-min_norm[vib_id])/(inter[1]-inter[0])
        if pbint in pb_vib_inv : vib = min_norm[vib_id] + (abs(crit)-abs(inter[1])) * (max_norm[vib_id]-min_norm[vib_id])/(abs(inter[0])-abs(inter[1]))
        ...
        if vib < min_norm[vib_id] : vib = 0

    return str(vib), vib_id #message to be sent to the vibrator and corresponding vibrator id 
